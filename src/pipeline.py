...
def convert_html_to_markdown(html):
    ...
    for tag, fmt in [('a','[{0}]({1})'), ('b','**{0}**'), ('strong','**{0}**'), ('i','*{0}*'), ('em','*{0}*')]:
...
def scrape_markdown(relevant_results):
    ...
    for result in relevant_results:
        url = result['link']
        response = requests.get("https://scraping.narf.ai/api/v1/", params={
            "api_key": os.environ['SCRAPING_API_KEY'],
            "url": url, "render_js": "true"
        })
        if response.ok:
            md = convert_html_to_markdown(response.text)
            markdowns.append({ 'url': url, 'title': result['title'], 'id': result['id'], 'markdown': md })
    return markdowns
...
def run_review_graph(summaries):
    ...
    summariser = ChatPromptTemplate.from_messages([("system", load_prompt("summariser")), ("placeholder", "{messages}")])
    summariser_chain = summariser | llm.with_structured_output(SummariserOutput)

    def summariser_fn(state: State):
        out = summariser_chain.invoke({"messages": state["messages"], "list_of_summaries": state["summaries"], "input_template": email_template})
        return {"messages": [AIMessage(content=out.email_summary), AIMessage(content=out.message)], "created_summaries": [out.email_summary]}

    class ReviewerOutput(BaseModel):
        approved: bool
        message: str

    reviewer = ChatPromptTemplate.from_messages([("system", load_prompt("reviewer")), ("placeholder", "{messages}")])
    reviewer_chain = reviewer | llm.with_structured_output(ReviewerOutput)

    def reviewer_fn(state: State):
        msgs = [HumanMessage(content=m.content) if isinstance(m, AIMessage) else AIMessage(content=m.content) for m in state["messages"]]
        state["messages"] = msgs
        out = reviewer_chain.invoke({"messages": state["messages"]})
        return {"messages": [HumanMessage(content=out.message)], "approved": out.approved}
