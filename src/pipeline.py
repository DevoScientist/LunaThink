...
#
#
# def load_prompt(prompt_name, profile="Business"):
#     with open(os.path.join(PROMPT_DIR, profile, f"{prompt_name}.md"), "r") as file:
#         return file.read()
#
#
...
#     prompt = load_prompt("relevance_check", profile)
...
#     prompt = load_prompt("summarise_markdown_page", profile)
...
#     email_template = load_prompt("email_template",profile)
#     llm = ChatOpenAI(model="gpt-4.1-mini")
#
...
#         [("system", load_prompt("summariser", profile)), ("placeholder", "{messages}")]
#     )
...
#         [("system", load_prompt("reviewer", profile)), ("placeholder", "{messages}")]
#     )
...
class PromptLoader:
    ...
    def load(self, prompt_name: str) -> str:
        path = os.path.join(self.base_dir, self.profile, f"{prompt_name}.md")
